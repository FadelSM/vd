--[ VIOLENCE DISTRICT - GUI PREMIUM ZEXSPARK - FadelSM]
...
function toggleOmegaLaser(state)
    if state then
        ...
        laserConnection = RunService.RenderStepped:Connect(function()
            if Settings.OmegaLaser and Character and Character:FindFirstChild("Head") then
        ...
        for _, v in pairs(Workspace:GetChildren()) do
            if v:IsA("Part") and v.Material == Enum.Material.Neon and v.Color == Color3.fromRGB(255, 0, 0) then
                v:Destroy()
            end
        end
    end
...
function bindSoul(target)
    if target and target.Character then
        ...
        if not soulBindConnection then
            soulBindConnection = Humanoid:GetPropertyChangedSignal("Health"):Connect(function()
                if Humanoid.Health <= 0 then
                    for _, soul in pairs(boundSouls) do
                        if soul and soul.Character and soul.Character:FindFirstChild("Humanoid") then
    ...
    local attraction = RunService.RenderStepped:Connect(function()
        for _, v in pairs(Players:GetPlayers()) do
            if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                ...
                local direction = (voidPart.Position - rootPart.Position).Unit
                rootPart.CFrame = rootPart.CFrame + direction * 25
                if (rootPart.Position - voidPart.Position).Magnitude < 15 then
...
function mapControl(action)
    for _, obj in pairs(Workspace:GetChildren()) do
        if obj:IsA("BasePart") and obj.Name ~= "Baseplate" then
            ...
            elseif action == "crush" and obj ~= Character and obj ~= Character:FindFirstChild("HumanoidRootPart") then
                obj:BreakJoints()
            end
        end
    end
...
local infiniteSkillsConnected = false
function toggleInfiniteSkills(state)
    if state and not infiniteSkillsConnected then
...
function toggleRewindOthers(state)
    if state then
        ...
        rewindConnection = RunService.RenderStepped:Connect(function()
            for _, v in pairs(Players:GetPlayers()) do
                if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
...
-- ========== MEMBUAT GUI PREMIUM (POSISI TENGAH - UKURAN SEDANG) ==========

local screenGui = Instance.new("ScreenGui")
screenGui.Name = "ZEXSPARK_PREMIUM_GUI"
...
function createPremiumSlider(labelText, setting, minVal, maxVal, currentVal, callback)
    ...
    UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 and moveConnection then
...
addPremiumSection("🔥 KOMBAT", Color3.fromRGB(139, 0, 255))
...
createPremiumToggle("💀 One Hit", "OneHit")
createPremiumToggle("🔫 Inf Ammo", "InfiniteAmmo")
createPremiumToggle("💚 Auto Heal", "AutoHeal")
createPremiumSlider("📏 Kill Range", "KillRange", 10, 200, Settings.KillRange)

-- SECTION 2: MOBILITAS
addPremiumSection("🏃 MOBILITAS", Color3.fromRGB(200, 100, 0))
createPremiumToggle("⚡ Speed Hack", "SpeedHack")
createPremiumSlider("🏃 Speed", "SpeedValue", 16, 250, Settings.SpeedValue)
createPremiumToggle("🕊️ Fly", "Fly")
createPremiumToggle("🧱 No Clip", "NoClip")
createPremiumToggle("👻 Invisible", "Invisible")
createPremiumToggle("🦘 Unl Jump", "UnlimitedJump")
createPremiumToggle("💫 Light Speed", "SpeedOfLight")

-- SECTION 3: VISUAL
addPremiumSection("🎯 VISUAL", Color3.fromRGB(0, 150, 200))
createPremiumToggle("👁️ ESP", "ESP")
createPremiumToggle("🌈 Rainbow", "RainbowTrail")
createPremiumToggle("🌀 Reality Warp", "RealityWarp")

-- SECTION 4: FITUR DEWA
addPremiumSection("💀 DEWA", Color3.fromRGB(200, 0, 200))
createPremiumToggle("⏰ Freeze", "TimeFreeze")
createPremiumToggle("🧲 Magnet", "MagnetPlayer")
createPremiumToggle("🧠 Mind Ctrl", "MindControl")
createPremiumToggle("💥 Damage x100", "DamageMultiplier")
createPremiumToggle("🛡️ Parry", "AutoParry")
createPremiumToggle("💢 Revenge", "RevengeMode")
createPremiumToggle("🌌 Dimension", "DimensionShift")
createPremiumToggle("🌀 Time Loop", "TimeLoop")

-- SECTION 5: LEGENDARY
addPremiumSection("⚡ LEGENDARY", Color3.fromRGB(255, 100, 0))
createPremiumToggle("🔫 Omega Laser", "OmegaLaser")
createPremiumToggle("♾️ Inf Skills", "InfiniteSkills")
createPremiumToggle("🎯 Tracker", "PlayerTracker")
createPremiumToggle("⏪ Rewind", "RewindOthers")

-- SECTION 6: BUTTON
addPremiumSection("💎 BUTTON", Color3.fromRGB(255, 215, 0))
createPremiumButton("🔗 Soul Bind", function()
    local closest = nil
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= LocalPlayer and v.Character then
            closest = v
...
createPremiumButton("💣 Nuker", function() inventoryNuker() end, Color3.fromRGB(200, 50, 50))
createPremiumButton("🌀 Void", function() gravityVoid() end, Color3.fromRGB(75, 0, 130))
createPremiumButton("🗺️ Rotate", function() mapControl("rotate") end, Color3.fromRGB(0, 150, 200))
createPremiumButton("🗺️ Flip", function() mapControl("flip") end, Color3.fromRGB(0, 150, 200))
createPremiumButton("🗺️ Crush", function() mapControl("crush") end, Color3.fromRGB(255, 50, 50))
createPremiumButton("🎤 Voice", function() voiceChanger() end, Color3.fromRGB(100, 200, 100))
createPremiumButton("💀 NUKE", function()
    for _, v in pairs(Players:GetPlayers()) do
...
createPremiumButton("👻 Army", function()
    for i = 1, 30 do
...
-- Notifikasi premium
StarterGui:SetCore("SendNotification", {
    Title = "✨ ZEXSPARK PREMIUM ✨",
    Text = "LEGENDARY EDITION | 58 FITUR",
    Duration = 3
})

-- Buka/tutup dengan INSERT
...
UserInputService.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.F and Settings.Fly and Character and Character:FindFirstChild("HumanoidRootPart") then
        flying = not flying
        if flying then
...
RunService.RenderStepped:Connect(function()
    if flying and Settings.Fly and Character and Character:FindFirstChild("HumanoidRootPart") then
